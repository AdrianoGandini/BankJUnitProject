...
class LoanServiceImplTest {
	...
	@ParameterizedTest
	@CsvSource({ "100.0, 50.0,  50.0", // loan, loan pay, expected final balance
			"200.0, 100.0, 100.0", // (starting balance is 100)
			"500.0, 300.0, 200.0", "50.0,  25.0,  25.0" })
	@DisplayName("Should handle multiple transactions correctly")
	void testMultipleTransactions(double loanAmount, double payAmount, double expectedBalance)
		...
		loanService.approveLoan(VALID_ACCOUNT_NAME, loanAmount);
		loanService.repayLoan(VALID_ACCOUNT_NAME, payAmount);

		double actualBalance = loanService.getLoan(VALID_ACCOUNT_NAME);
